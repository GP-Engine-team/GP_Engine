...
void ImGuiSaveStyle(const char* filename, const ImGuiStyle& style)
{
    ...
    {
        Log::getInstance()->logError(stringFormat("The file \"%s\" was not opened to write", filename));
        return;
    }
    ...
    Log::getInstance()->logInitializationEnd("Style exported");
}
...
void ImGuiLoadStyle(const char* filename, ImGuiStyle& style)
{
    ...
    // Release memory
    ImGui::MemFree(f_data);
    Log::getInstance()->logInitializationEnd("Style imported");
}
...
bool ShowStyleSelector(const char* label)
{
    ...
    if (ImGui::Combo(label, &style_idx, "Dark\0Light\0Classic\0GPStyle\0"))
    {
        switch (style_idx)
        {
        case 0:
