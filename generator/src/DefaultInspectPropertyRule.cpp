...
std::string DefaultInspectPropertyRule::generateClassFooterCode(
    kodgen::EntityInfo const& entity, kodgen::ComplexProperty const& property,
    ...
    const std::string& argClassName, const std::string& fieldCallingFunction, std::string extraQualifier) const
{
    ...
    if (property.subProperties.empty() || property.subProperties[0] == "true")
    {
        ...
        defaultInspectInside += callParents;
    }
    ...
    for (auto& field : entity.fields)
    {
        ...
        auto isPropertyReflected = [&](const kodgen::ComplexProperty& prop) { return prop.mainProperty == "Inspect"; };
