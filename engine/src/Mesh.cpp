...
Mesh::Mesh(const CreateArg& arg, bool isLoadInGPU)
    :   m_indexVAO       (0),
        m_isLoadInGPU    (false),
        m_objName        (arg.objName),
        m_vBuffer        (arg.vBuffer),
        m_vtBuffer       (arg.vtBuffer),
        m_vnBuffer       (arg.vnBuffer),
        m_iBuffer        (arg.iBuffer)
{
    if (isLoadInGPU)
        loadInGPU();
}
...
Mesh::CreateArg Mesh::createPlane	(float textureRepetition, unsigned int indexTextureX, unsigned int indexTextureY, Axis towardAxis)
{
	Mesh::CreateArg mesh;
    mesh.objName = "Plane";
