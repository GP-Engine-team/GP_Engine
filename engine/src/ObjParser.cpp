...
inline
void parseVertexPos (const std::string& line, std::vector<Vec3>& vBuffer)
{
...
inline
void parseTexturePos (const std::string& line, std::vector<Vec2>& vtBuffer)
{
...
inline
void parseNormal (const std::string& line, std::vector<Vec3>& vnBuffer)
{
    std::size_t cursorStart = line.find_first_not_of(" ", 2);
    std::size_t cursorEnd = line.find(" ", cursorStart);
    float x = std::stof(line.substr(cursorStart, cursorEnd - cursorStart));
...
inline
unsigned int getNumberIndice (const std::string& line, unsigned int numberAttributeByIndice)
{
    return numberAttributeByIndice == 1 ? 0 : std::count(line.begin(), line.end(), '/') / (numberAttributeByIndice - 1);
}

inline
unsigned int getNumberAttributeByIndice (const std::string& line)
{
    std::size_t cursorStart = line.find_first_not_of(" ", 1);
...
inline
void parseIndice (const std::string& line, std::vector<Shape>& shape)
{
...
inline
void parseMtl (const std::string& line, std::vector<MaterialAttrib>& materials)
{
    std::string path;
...
inline
void parseUseMtl (const std::string& line, std::vector<Shape>& shape)
{
    shape.push_back({});
...
inline
void parseSurfaceName (const std::string& line, std::vector<Shape>& shape)
{
   if (!shape.empty())
...
inline
void parseLine (const std::string& line, Attrib* attrib, std::vector<Shape>* shape, std::vector<MaterialAttrib>* materials)
{
    //Detect line type and parse this line
    if      (isTitle(line, "#"))                                return;
    else if (isTitle(line, "o") && attrib != nullptr)           {parseName(line, attrib->objName);
                                                                Log::log((std::string("Obj name : ") + attrib->objName).c_str()); }
    else if (isTitle(line, "g") && shape != nullptr)            parseSurfaceName(line, *shape);
    else if (isTitle(line, "mtllib") && materials != nullptr)   parseMtl(line, *materials);
    else if (isTitle(line, "usemtl") && shape != nullptr)       parseUseMtl(line, *shape);
    else if (isTitle(line, "vt") && attrib != nullptr)          parseTexturePos(line, attrib->vtBuffer);
    else if (isTitle(line, "vn") && attrib != nullptr)          parseNormal(line, attrib->vnBuffer);
    else if (isTitle(line, "v") && attrib != nullptr)           parseVertexPos(line, attrib->vBuffer);
    else if (isTitle(line, "f") && shape != nullptr)            parseIndice(line, *shape);
    else                                                        Log::logWarning(std::string("Obj Parser don't parse this line : " + line).c_str());
}

void Engine::Core::Parsers::loadObjWithMTL (const char* objPath, Attrib* attrib, std::vector<Shape>* shape, std::vector<MaterialAttrib>* materials) noexcept
{
